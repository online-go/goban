...
export class Goban extends TypedEventEmitter<Events> {
    ...
    public static setHooks(hooks:GobanHooks):void {
        for (let name in hooks) {
            Goban.hooks[name] = hooks[name];
        }
    }

    protected getClockDrift():number {
        if (Goban.hooks.getClockDrift) {
            return Goban.hooks.getClockDrift();
        }
        console.warn("getClockDrift not provided for Goban instance");
        return 0;
    }
    protected getNetworkLatency():number {
        if (Goban.hooks.getNetworkLatency) {
            return Goban.hooks.getNetworkLatency();
        }
        console.warn("getNetworkLatency not provided for Goban instance");
        return 0;
    }
    protected getCoordinateDisplaySystem():'A1'|'1-1' {
        if (Goban.hooks.getCoordinateDisplaySystem) {
            return Goban.hooks.getCoordinateDisplaySystem();
        }
        return 'A1';
    }
    protected getShowMoveNumbers():boolean {
        if (Goban.hooks.getShowMoveNumbers) {
            return Goban.hooks.getShowMoveNumbers();
        }
        return false;
    }
    protected getShowVariationMoveNumbers():boolean {
        if (Goban.hooks.getShowVariationMoveNumbers) {
            return Goban.hooks.getShowVariationMoveNumbers();
        }
        return false;
    }
    public static getMoveTreeNumbering():string {
        if (Goban.hooks.getMoveTreeNumbering) {
            return Goban.hooks.getMoveTreeNumbering();
        }
        return 'move-number';
    }
    public static getCDNReleaseBase():string {
        if (Goban.hooks.getCDNReleaseBase) {
            return Goban.hooks.getCDNReleaseBase();
        }
        return '';
    }
    public static getSoundEnabled():boolean {
        if (Goban.hooks.getSoundEnabled) {
            return Goban.hooks.getSoundEnabled();
        }
        return true;
    }
    public static getSoundVolume():number {
        if (Goban.hooks.getSoundVolume) {
            return Goban.hooks.getSoundVolume();
        }
        return 0.5;
    }
    protected defaultConfig():any {
        if (Goban.hooks.defaultConfig) {
            return Goban.hooks.defaultConfig();
        }
        return {};
    }
    protected watchSelectedThemes(cb):{ remove:() => any } {
        if (Goban.hooks.watchSelectedThemes) {
            return Goban.hooks.watchSelectedThemes(cb);
        }
        return { remove: () => {} };
    }
    protected isAnalysisDisabled(perGameSettingAppliesToNonPlayers:boolean = false):boolean {
        if (Goban.hooks.isAnalysisDisabled) {
            return Goban.hooks.isAnalysisDisabled(this, perGameSettingAppliesToNonPlayers);
        }
        return false;
    }

    protected getLocation():string {
        if (Goban.hooks.getLocation) {
            return Goban.hooks.getLocation();
        }
        return window.location.pathname;
    }
    protected getSelectedThemes() {
        if (Goban.hooks.getSelectedThemes) {
            return Goban.hooks.getSelectedThemes();
        }
